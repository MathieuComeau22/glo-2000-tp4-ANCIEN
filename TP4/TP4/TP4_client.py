...
class Client:
    ...
    def _recv_data(self) -> TP4_utils.GLO_message:
        ...
        """
        # TODO
        message = json.loads(glosocket.recv_msg(self._socket))
        
        if message is None :
            print("error in _recv_data() : message is None")
            exit()
        if message["header"] is None :
            print("error in _recv_data() : message has no header")
            exit()
        if message["data"] is None :
            print("error in _recv_data() : message has no data")
            exit()
    ...
    def _authentication(self) -> None:
        ...
        if choice == "1" or choice == "2" :
            ...
            glosocket.send_msg(destination=self._socket, message=json.dumps(TP4_utils.GLO_message(message)))
    ...
    def _reading(self) -> None:
        ...
        if len(response1["data"]) == 0 :
            print("Aucun courriels à consulter.")
            return

        for email in response1["data"] :
            print(email)
        choice = input("Choisir un numéro de courriel : ")

        message = {
                "header": TP4_utils.message_header.INBOX_READING_CHOICE,
                "data": {"username": self._username, "mailNumber": choice}
            }
        glosocket.send_msg(destination=self._socket, message=json.dumps(TP4_utils.GLO_message(message)))

        response2 = self._recv_data()
        if response2["header"] == TP4_utils.message_header.OK :
            print(TP4_utils.EMAIL_DISPLAY.format(
                source=response2['data']["source"], 
                destination=response2['data']["destination"], 
    ...
    def _get_stats(self) -> None:
        ...
        response = self._recv_data()
        if response["header"] == TP4_utils.message_header.OK :
            print(TP4_utils.STATS_DISPLAY.format(count=response["data"]["count"], size=response["data"]["folder_size"]))
        else :
            print(response["data"])
